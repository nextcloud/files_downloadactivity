OC.L10N.register(
    "files_downloadactivity",
    {
    "Downloaded by {actor} (via desktop)" : "Λήψη από {χρήστη} (μέσω επιφάνειας εργασίας)",
    "Downloaded by {actor} (via app)" : "Λήψη από {actor} (μέσω εφαρμογής)",
    "Downloaded by {actor} (via browser)" : "Λήψη από {actor} (μέσω προγράμματος περιήγησης)",
    "Shared file {file} was downloaded by {actor} via the desktop client" : "Το κοινόχρηστο αρχείο {file} μεταφορτώθηκε από {actor} μέσω του προγράμματος-πελάτη επιφάνειας εργασίας",
    "Shared file {file} was downloaded by {actor} via the mobile app" : "Το κοινόχρηστο αρχείο {file} ανακτήθηκε από {actor} μέσω της εφαρμογής για κινητά",
    "Shared file {file} was downloaded by {actor} via the browser" : "Το κοινόχρηστο αρχείο {file} λήφθηκε από τον {actor} μέσω του περιηγητή",
    "A local shared file or folder was <strong>downloaded</strong>" : "Ένα τοπικό διαμοιρασμένο αρχείο ή φάκελος έχει <strong>ληφθεί</strong>",
    "Activities for shared file downloads" : "Δραστηριότητες σχετικά με τις λήψεις κοινόχρηστων αρχείων",
    "Downloaded by %s (via desktop)" : "Λήψη από τον χρήστη %s (μέσω υπολογιστή)",
    "Downloaded by %s (via mobile)" : "Λήψη από τον χρήστη %s (μέσω κινητού)",
    "Downloaded by {actor} (via mobile)" : "Λήψη από {χρήστη} (δια μέσω κινητού)",
    "Downloaded by %s (via web)" : "Λήψη από τον χρήστη %s (μέσω ιστού)",
    "Downloaded by {actor} (via web)" : "Λήψη από {χρήστη} (δια μέσω ιστού)",
    "Shared file %1$s was downloaded by %2$s via the desktop client" : "Το κοινόχρηστο αρχείο %1$sλήθφηκε από τον %2$sμέσω της εφαρμογής για υπολογιστή",
    "Shared file %1$s was downloaded by %2$s via the mobile client" : "Το κοινόχρηστο αρχείο %1$sλήφθηκε από τον χρήστη %2$sμέσω της εφαρμογής για κινητά",
    "Shared file {file} was downloaded by {actor} via the mobile client" : "Το κοινόχρηστο αρχείο {file} λήφθηκε από τον {actor} μέσω της εφαρμογής για κινητά",
    "Shared file %1$s was downloaded by %2$s via the web interface" : "Το κοινόχρηστο αρχείο %1$sλήφθηκε από τον χρήστη %2$sμέσω γραφικής διασύνδεσης ιστού",
    "Shared file {file} was downloaded by {actor} via the web interface" : "Το κοινόχρηστο αρχείο {file} λήφθηκε από τον χρήστη {actor} μέσω της γραφικής διασύνδεσης ιστού"
},
"nplurals=2; plural=(n != 1);");
